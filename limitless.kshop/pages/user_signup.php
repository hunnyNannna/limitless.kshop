...
<body class="login-page sidebar-collapse">
    ...
    <?php
        ...
        {   
            ...
            if(empty($fullname) || empty($email) || empty($address) || empty($phoneno) || empty($username) || empty($password)) {    
                ...
                //updating the table
                $query = "INSERT INTO users (fullname, email, address, phoneno, username, password) 
                                    VALUES ('$fullname','$email','$address','$phoneno','$username','$pass1')";

                $result = mysqli_query($dbconn,$query);
                if($result){
                    //redirecting to the display page. In our case, it is index.php

                    $query=mysqli_query($dbconn,"SELECT * FROM `users` WHERE username='$username' AND password='$pass1'");
