...
Class Master extends DBConnection {
	...
	function save_booking(){
		...
		$check = $this->conn->query("SELECT * FROM `booking_list` WHERE `facility_id` = '{$facility_id}' AND (DATE('{$date_from}') BETWEEN DATE(`date_from`) AND DATE(`date_to`) OR DATE('{$date_to}') BETWEEN DATE(`date_from`) AND DATE(`date_to`) OR (DATE(`date_from`) BETWEEN DATE('{$date_from}') AND DATE('{$date_to}') AND TIME(`time`) >= TIME('{$time_from}')) OR (DATE(`date_to`) BETWEEN DATE('{$date_from}') AND DATE('{$date_to}') AND TIME(`time`) <= TIME('{$time_to}'))) AND `status` = 1")->num_rows;
